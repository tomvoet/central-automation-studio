...
var centralURLs = [
  {
    ...
    "https://apigw-prod2.central.arubanetworks.com":
      "https://app-prod2-ui.central.arubanetworks.com",
    ...
    "https://apigw-eucentral2.central.arubanetworks.com":
      "https://app-eucentral2.central.arubanetworks.com",
...
function populateCentralClusters() {
  for (let k in centralClusters[0]) {
    $("#clusterselector").append(
      $("<option>", { value: centralClusters[0][k], text: k })
    );
  }
}
...
function isCOPSelected() {
  ...
  if (
    document.getElementById("clusterselector").value ===
    getAPIGateway("Central On-Prem")
  ) {
...
function onFinishSetup() {
  ...
  localStorage.setItem(
    "port_variable_format",
    $("#port_variable_format").val()
  );
  localStorage.setItem("refresh_rate", $("#refresh_rate").val());
  localStorage.setItem(
    "load_clients",
    document.getElementById("load_clients").value === "Include" ? true : false
  );
  localStorage.setItem(
    "load_group_properties",
    document.getElementById("load_group_properties").value === "Include"
      ? true
      : false
  );
  localStorage.setItem(
    "load_airmatch_events",
    document.getElementById("load_airmatch_events").value === "Include"
      ? true
      : false
  );
...
function loadMonitoringData(refreshrate) {
  ...
  if (!localStorage.getItem("monitoring_update")) {
    ...
    var lastRefresh = new Date(
      parseInt(localStorage.getItem("monitoring_update"))
    );
    var now = new Date();
    ...
    if (diffMinutes > refreshrate) {
      ...
      updateGroupUI();
      wirelessClients = JSON.parse(
        localStorage.getItem("monitoring_wirelessClients")
      );
      wiredClients = clients.concat(
        JSON.parse(localStorage.getItem("monitoring_wiredClients"))
      );
  ...
  $("#files").parse({
    ...
    complete: function () {
      if (clickedRow === "mgmt-group") {
        ...
        showNotification(
          "ca-c-warning",
          "No CSV data found. Try selecting a CSV document.",
          "bottom",
          "center",
          "danger"
        );
...
function generateCSVForSite(clickedRow) {
  ...
  if (!selectedSite) {
    showNotification(
      "ca-c-warning",
      "Please select a Site before running a task",
      "bottom",
      "center",
      "danger"
    );
    ...
    $.each(aps, function () {
      if (this["site"] === selectedSite) {
        csvData.push({
          [nameKey]: this["name"],
          [serialKey]: this["serial"],
          [macKey]: this["macaddr"],
          [groupKey]: this["group_name"],
          [siteKey]: this["site"],
        });
      }
    });
    ...
    if (!csvData) {
      showNotification(
        "ca-c-warning",
        "No site devices found. Try selecting a different site?",
        "bottom",
        "center",
        "danger"
      );
    ...
    if (clickedRow === "adddevices") {
      ...
      $.each(aps, function () {
        if (
          ...
          (this["model"].includes("344") || this["model"].includes("345"))
        ) {
          csvData.push({
            [nameKey]: this["name"],
            [serialKey]: this["serial"],
...
function tokenRefresh() {
  ...
  return $.ajax(settings)
    .done(function (commandResults, statusText, jqXHR) {
      ...
      if (
        commandResults.hasOwnProperty("status") &&
        commandResults.status === "503"
      ) {
        apiErrorCount++;
        logError(
          "Central Server Error (503): " +
            commandResults.reason +
            " (/auth/refresh)"
        );
      } else if (commandResults.hasOwnProperty("error")) {
        Swal.fire({
          title: "Central API connection failed",
          text: response.error_description.replace(
            "refresh_token",
            "Refresh Token"
          ),
          icon: "error",
    ...
    .fail(function (XMLHttpRequest, textStatus, errorThrown) {
      ...
      if (XMLHttpRequest.readyState == 4) {
        ...
        showNotification(
          "ca-globe",
          XMLHttpRequest.statusText.replace("refresh_token", "Refresh Token"),
          "bottom",
          "center",
          "danger"
        );
...
function updateAPILimits(headers) {
  if (headers["X-RateLimit-Limit-day"]) {
    var usage =
      parseInt(headers["X-RateLimit-Limit-day"]) -
      parseInt(headers["X-RateLimit-Remaining-day"]);
    $("#titleText").attr(
      "data-original-title",
      "Daily API Usage: " +
        usage.toString() +
...
function updateMonitoringWithClients(needClients) {
  ...
  if (
    ...
    showNotification(
      "ca-globe",
      "API settings are blank...",
      "bottom",
      "center",
      "danger"
    );
    window.location.href =
      window.location.href.substr(0, location.href.lastIndexOf("/") + 1) +
      "settings.html";
  } else if (
    localStorage.getItem("base_url") &&
    !localStorage.getItem("account_details")
  ...
  $.ajax(settings)
    .done(function (commandResults, statusText, jqXHR) {
      ...
      if (
        ...
        wiredClients = [];
        wirelessClients = [];
        if (document.getElementById("client_count"))
          document.getElementById("client_count").innerHTML = "0";
...
function getLicensingStats() {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/licensing/v1/subscriptions/stats?license_type=all",
      access_token: localStorage.getItem("access_token"),
    }),
  };
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response) {
      if (response.expiring) {
        showNotification(
          "ca-license-key",
          "A Subscription Key expiring soon...",
          "bottom",
          "center",
          "warning"
        );
...
function loadClientsUI(client) {
  ...
  if (
    !client["health"] &&
    client["failure_stage"] !== "" &&
    client["failure_stage"] !== "NA"
  ) {
    status =
      '<span data-toggle="tooltip" data-placement="right" title="Failed To Connect: ' +
      client["failure_reason"] +
      " at " +
      client["failure_stage"] +
      '"><i class="fa fa-circle text-danger"></i></span>';
  } else if (!client["health"]) {
    status = '<i class="fa fa-circle text-neutral"></i>';
  } else if (client["health"] < 50) {
    status =
      '<span data-toggle="tooltip" data-placement="right" title="Health: ' +
      client["health"] +
      '"><i class="fa fa-circle text-danger"></i></span>';
    ...
    status =
      '<span data-toggle="tooltip" data-placement="right" title="Health: ' +
      client["health"] +
      '"><i class="fa fa-circle text-warning"></i></span>';
  } else {
    status =
      '<span data-toggle="tooltip" data-placement="right" title="Health: ' +
      client["health"] +
  ...
  var clientURL =
    centralURLs[0][apiURL] +
    "/frontend/#/CLIENTDETAIL/" +
    client["macaddr"] +
    "?ccma=" +
    client["macaddr"] +
    "&cdcn=" +
  ...
  table.row.add([
    ...
    ip_address,
    os_type,
    associatedDevice_name,
    site,
    `<strong onclick="setVlan('${client_mac}')" style="cursor: pointer;">${vlan}</strong>`,
  ]);
...
/*
async function setVlan(client_mac) {
  const clearPassUrl = "Replace with ClearPass URL";
  const clearPassToken = "Replace with ClearPass Token";

  if (!confirm("Do you really want to set the VLAN?")) return;

  const vlanID = prompt("Enter the VLAN ID", "1");
  ...
  try {
    await changeClientVlan(client_mac, vlanID, {
      clearPassUrl,
      clearPassToken,
    });

    const sessionID = await getSessionID(client_mac, {
      clearPassUrl,
      clearPassToken,
    });

    await reauthorizeSession(sessionID, {
      clearPassUrl,
      clearPassToken,
    });

    alert("VLAN changed successfully");
  } catch (e) {
    alert(e.message);
    return;
  }
...
async function setVlan(client_mac) {
  ...
  const res = await fetch(getAPIURL() + "/set_vlan", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      ...
      vlan: vlanID,
      api_base: clearPassUrl,
      access_token: clearPassToken,
    }),
  });

  if (!res.ok) {
    throw new Error("Failed to change VLAN");
  }
...
function normalizeMac(unNormalizedMac) {
  let normalizedMac = unNormalizedMac.replaceAll(":", "").replaceAll("-", "");
  normalizedMac = normalizedMac.toLowerCase();

  return normalizedMac;
}

/*
async function changeClientVlan(client_mac, vlan_id, req_args) {
  const { clearPassUrl, clearPassToken } = req_args;

  const res = await fetch(
    `${clearPassUrl}/api/endpoint/mac-address/${client_mac}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${clearPassToken}`,
      },
      body: JSON.stringify({
        attributes: {
          vlan: vlan_id,
...
async function getSessionID(client_mac, req_args) {
  const { clearPassUrl, clearPassToken } = req_args;

  const res = await fetch(`${clearPassUrl}/api/session`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${clearPassToken}`,
  ...
  const sessions = data._embedded.items;
  ...
  return sessions.filter(
    (session) => session.callingstationid === normalizedMac
  )[0].id;
}

async function reauthorizeSession(session_id, req_args) {
  const { clearPassUrl, clearPassToken } = req_args;

  const res = await fetch(
    `${clearPassUrl}/api/session/${session_id}/reauthorize`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!res.ok) {
    throw new Error("Failed to reauthorize session");
  }
}
*/
function updateClientUI() {
  ...
  if (document.getElementById("client_count"))
    document.getElementById("client_count").innerHTML =
      "" + clients.length + "";
...
function getWirelessClientData(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/monitoring/v2/clients?calculate_total=true&offset=" +
        offset +
        "&limit=" +
        apiLimit +
        "&timerange=3H&client_type=WIRELESS&client_status=CONNECTED&show_usage=true&show_manufacturer=true&show_signal_db=true",
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      try {
        ...
        console.log(
          "Browser Storage Full. Not able to cache client information"
        );
      }
    } else {
      ...
      if (offset + apiLimit < response.total)
        getWirelessClientData(offset + apiLimit);
      else {
        updateClientUI();
        try {
          localStorage.setItem(
            "monitoring_wirelessClients",
            JSON.stringify(wirelessClients)
          );
        } catch (e) {
          console.log(
            "Browser Storage Full. Not able to cache client information"
...
function getWiredClientData(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        ...
        "&limit=" +
        apiLimit +
        "&timerange=3H&client_type=WIRED&client_status=CONNECTED&show_usage=true&show_manufacturer=true",
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiLimit < response.total)
        getWiredClientData(offset + apiLimit);
      else {
        updateClientUI();
        try {
          localStorage.setItem(
            "monitoring_wiredClients",
            JSON.stringify(wiredClients)
          );
        } catch (e) {
          console.log(
            "Browser Storage Full. Not able to cache client information"
...
function loadAPUI(ap) {
  //console.log(ap);
  var memoryUsage = (
    ((ap["mem_total"] - ap["mem_free"]) / ap["mem_total"]) *
    100
  )
    .toFixed(0)
    .toString();
  ...
  if (ap["status"] == "Up") {
    status =
      '<span data-toggle="tooltip" data-placement="right" data-html="true" title="CPU Usage: ' +
      ap["cpu_utilization"] +
      "%<br>Memory Usage:" +
      memoryUsage +
      '%"><i class="fa fa-circle text-success"></i></span>';
...
function updateAPUI() {
  ...
  if (document.getElementById("ap_count"))
    document.getElementById("ap_count").innerHTML = "" + aps.length + "";
...
function getAPData(offset, needClients) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/monitoring/v2/aps?calculate_total=true&show_resource_details=true&calculate_client_count=true&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
  ...
  return $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiLimit < response.total)
        getAPData(offset + apiLimit, needClients);
      else {
        updateAPUI();
        try {
...
function getBSSIDData(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/monitoring/v2/bssids?calculate_total=true&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
    ...
    if (response.hasOwnProperty("message")) {
      ...
      if (offset + apiLimit < response.total) getBSSIDData(offset + apiLimit);
        ...
        try {
          ...
          console.log(
            "Browser Storage Full. Not able to cache BSSID information"
          );
        }
      }
    }
...
function loadSwitchUI(device) {
  var memoryUsage = (
    ((device["mem_total"] - device["mem_free"]) / device["mem_total"]) *
    100
  )
    .toFixed(0)
    .toString();
...
function updateSwitchUI() {
  ...
  if (document.getElementById("switch_count"))
...
function getSwitchData(offset, needClients) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/monitoring/v1/switches?calculate_total=true&show_resource_details=true&calculate_client_count=true&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset == 0) {
        ...
        if (page.includes("experimental-switching"))
          $("#chassis-switch-table").DataTable().rows().remove();
      }
      ...
      if (offset + apiLimit < response.total)
        getSwitchData(offset + apiLimit, needClients);
      else {
        updateSwitchUI();
        try {
          ...
          console.log(
            "Browser Storage Full. Not able to cache Switch information"
          );
        }
...
function loadGatewayUI(device) {
  ...
  var clientURL =
    centralURLs[0][apiURL] +
    "/frontend/#/GATEWAYDETAIL/OVERVIEW/" +
    device["serial"] +
    "?csg=" +
    device["serial"] +
    "&cdcn=" +
...
function updateGatewayUI() {
  ...
  if (document.getElementById("gateway_count"))
...
function getGatewayData(offset) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/monitoring/v1/gateways?calculate_total=true&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiLimit < response.total) getGatewayData(offset + apiLimit);
        ...
        try {
          ...
          console.log(
            "Browser Storage Full. Not able to cache Gateway information"
          );
        }
      }
    }
...
function loadSiteUI(site) {
  ...
  var clientURL =
    centralURLs[0][apiURL] +
    "/frontend/#/SITEHEALTH?id=" +
    site["id"] +
    "&name=" +
    name +
    "&cid=2&cn=Site&l=label&nc=site";
  var aiURL =
    centralURLs[0][apiURL] +
    "/frontend/#/SITE_INSIGHTS?id=" +
    site["id"] +
    "&name=" +
    name +
    "&cid=2&cn=Site&l=label&nc=site";
  ...
  table.row.add([
    ...
    capestate,
    '<a href="' + aiURL + '" target="_blank">' + aiinsights + "</a>",
    healthReason,
  ]);
  ...
  if ($(".selectpicker")) {
    ...
    $("#siteselector").append(
      $("<option>", { value: site["name"], text: site["name"] })
    );
...
function updateSiteUI() {
  ...
  if (document.getElementById("site_count"))
    document.getElementById("site_count").innerHTML = "" + sites.length + "";
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiSiteLimit < response.total) {
        ...
        try {
          ...
          console.log(
            "Browser Storage Full. Not able to cache Site information"
          );
        }
      }
    }
...
function loadGroupUI(group) {
  ...
  var wiredTemplateUsed =
    '<button class="btn btn-round btn-outline btn-tag">UI Group</button>';
  if (group.template_details && group.template_details["Wired"])
    wiredTemplateUsed =
      '<button class="btn btn-round btn-tag">Template</button>';
  var wirelessTemplateUsed =
    '<button class="btn btn-round btn-outline btn-tag">UI Group</button>';
  if (group.template_details && group.template_details["Wireless"])
    wirelessTemplateUsed =
      '<button class="btn btn-round btn-tag">Template</button>';

  var groupProperties = "";
  if (group.group_properties) {
    if (
      group.group_properties["AllowedDevTypes"].includes("AccessPoints") ||
      group.group_properties["AllowedDevTypes"].includes("Gateways")
    ) {
      if (group.group_properties["AOSVersion"] === "AOS_10X")
        groupProperties +=
          '<button class="btn btn-round btn-tag btn-warning">AOS 10</button>';
      if (group.group_properties["AOSVersion"] === "AOS_8X")
        groupProperties +=
          '<button class="btn btn-round btn-tag btn-element">AOS 8</button>';
    }

    if (group.group_properties["AllowedDevTypes"].includes("AccessPoints")) {
      if (
        group.group_properties["AOSVersion"] === "AOS_10X" &&
        group.group_properties["ApNetworkRole"] === "Standard"
      )
        groupProperties +=
          '<button class="btn btn-round btn-tag">Campus AP</button>';
      if (
        group.group_properties["AOSVersion"] === "AOS_10X" &&
        group.group_properties["ApNetworkRole"] === "Microbranch"
      )
        groupProperties +=
          '<button class="btn btn-round btn-tag">Microbranch AP</button>';
    }

    if (group.group_properties["AllowedDevTypes"].includes("Switches")) {
      if (group.group_properties["AllowedSwitchTypes"].includes("AOS_CX"))
        groupProperties +=
          '<button class="btn btn-round btn-tag">AOS-CX</button>';
      if (group.group_properties["AllowedSwitchTypes"].includes("AOS_S"))
        groupProperties +=
          '<button class="btn btn-round btn-tag">AOS-S</button>';
      if (group.group_properties["MonitorOnlySwitch"])
        groupProperties +=
          '<button class="btn btn-round btn-tag">Monitor-Only</button>';
    }

    if (group.group_properties["AllowedDevTypes"].includes("Gateways")) {
      if (group.group_properties["GwNetworkRole"] === "BranchGateway")
        groupProperties +=
          '<button class="btn btn-round btn-tag">Branch Gateway</button>';
      if (group.group_properties["GwNetworkRole"] === "WLANGateway")
        groupProperties +=
          '<button class="btn btn-round btn-tag">Mobility Gateway</button>';
      if (group.group_properties["GwNetworkRole"] === "VPNConcentrator")
        groupProperties +=
          '<button class="btn btn-round btn-tag">VPNC</button>';
    }
  }
  ...
  if ($("#groupselector")) {
    ...
    $("#groupselector").append(
      $("<option>", { value: group["group"], text: group["group"] })
    );
...
function updateGroupUI() {
  ...
  if (document.getElementById("group_count"))
    document.getElementById("group_count").innerHTML = "" + groups.length + "";
...
function getGroupTemplateInfo(currentGroups, last) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/configuration/v2/groups/template_info?groups=" +
        groupList,
      access_token: localStorage.getItem("access_token"),
    }),
  };
      ...
      if (loadGroupProperties) {
        ...
        if (last) {
          ...
          try {
            ...
            console.log(
              "Browser Storage Full. Not able to cache Group information"
            );
          }
...
function getGroupProperties(groupList, last) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/configuration/v1/groups/properties?groups=" +
        groupList,
      access_token: localStorage.getItem("access_token"),
    }),
  };
...
function updateInventory() {
  ...
  $.when(getAPInventory(0), getSwitchInventory(0), getGatewayInventory(0)).then(
    function () {
...
function getAPInventory(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/device_inventory/v1/devices?sku_type=all_ap&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
...
function getSwitchInventory(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/device_inventory/v1/devices?sku_type=switch&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiLimit < response.total)
        getSwitchInventory(offset + apiLimit); // if there are still objects to get
    }
  });
...
function getGatewayInventory(offset) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/device_inventory/v1/devices?sku_type=gateway&limit=" +
        apiLimit +
        "&offset=" +
        offset,
      access_token: localStorage.getItem("access_token"),
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (offset + apiLimit < response.total)
        getGatewayInventory(offset + apiLimit); // if there are still objects to get
...
function addDevices() {
  ...
  $.each(csvData, function () {
    ...
    if (currentClusterName === "Central On-Prem")
      devices.push({
        mac: cleanMACAddress(this["MAC"]),
        serial: this["SERIAL"].trim(),
        partNumber: this["MODEL"].trim(),
      });
    else
      devices.push({
        mac: cleanMACAddress(this["MAC"]),
  ...
  $.ajax(settings).done(function (response, textStatus, jqXHR) {
    ...
    if (response.code && response.code === "ATHENA_ERROR_NO_DEVICE") {
      if (
        response.extra.message.invalid_device &&
        response.extra.message.invalid_device.length > 0
      ) {
          ...
          if (this.status === "ATHENA_ERROR_DEVICE_ALREADY_EXIST") {
            logError(
              'Device with Serial number "' +
                this.serial +
                '" is already added to Central or is an invalid MAC address'
            );
          } else if (this.status === "INVALID_MAC_SN") {
            logError(
              'Device with Serial number "' + this.serial + '" is invalid'
            );
          } else {
            logError(
              'Device with Serial number "' + this.serial + '" is invalid'
      ...
      if (
        response.extra.message.blocked_device &&
        response.extra.message.blocked_device.length > 0
      ) {
        // invalid device - log reason
        $.each(response.extra.message.blocked_device, function () {
          if (this.status === "ATHENA_ERROR_DEVICE_ALREADY_EXIST") {
            logError(
              'Device with Serial number "' +
                this.serial +
                '" is already added to your Central account'
            );
          } else {
            logError(
              'Device with Serial number "' +
                this.serial +
                '" is blocked from being added to your Central account'
            );
          }
          apiErrorCount++;
        });
...
function licenseDevicesFromCSV(msp) {
  licenseNotification = showProgressNotification(
    "ca-license-key",
    "Licensing devices...",
    "bottom",
    "center",
    ...
    if (this["SERIAL"]) {
      ...
      if (deviceType === "IAP") {
        ...
        if (requestedLicense.toLowerCase().includes("wlan")) {
          ...
          requestedLicense.toLowerCase().includes("security") &&
          requestedLicense.toLowerCase().includes("foundation-base")
        ) {
          ...
          requestedLicense.toLowerCase().includes("security") &&
          requestedLicense.toLowerCase().includes("foundation") &&
          (arubaPart.includes("70") || arubaPart.includes("90"))
        ) {
          license = "foundation_90xx_sec";
        } else if (
          requestedLicense.toLowerCase().includes("security") &&
          requestedLicense.toLowerCase().includes("advanced") &&
          (arubaPart.includes("70") || arubaPart.includes("90"))
        ) {
      ...
      if (!foundDevice) {
        logError(
          "Device with Serial Number: " +
            currentSerial +
            " was not found in the device inventory"
        );
...
function unlicenseDevicesFromCSV() {
  licenseCounter = 0;
  licenseNotification = showProgessNotification(
    "ca-license-key",
    "Un-assigning licenses from devices",
    "bottom",
    "center",
    "info"
  );
  ...
  $.each(csvData, function () {
    if (this["SERIAL"]) {
      ...
      if (!device) {
        logError(
          "Unable to find device " + this["SERIAL"] + " in the device inventory"
        );
        apiErrorCount++;
        licenseCounter++;
...
function selectGroup() {
  ...
  document.getElementById("manualGroupParagraph").innerText =
  ...
  showNotification(
    ...
    "The " +
      manualGroup +
      " group will be used for devices with no group assigned. Please re-run the task.",
    "top",
    "center",
    "warning"
  );
...
function moveDevicesToGroup() {
  /*  
	...
	*/
  moveNotification = showProgressNotification(
    "ca-folder-replace",
    "Moving devices into groups...",
    "bottom",
    "center",
    "info"
  );
  ...
  for (const [groupName, serialsToMove] of Object.entries(groupsToUse)) {
    ...
    while (serialArray.length > 0) {
      ...
      logInformation(
        "Adding Devices to " + groupName + ": " + JSON.stringify(serialBlock)
      );
      ...
      $.when(performDeviceMove(groupName, serialBlock, new $.Deferred())).then(
        function () {
...
function performDeviceMove(groupName, serialNumbers, movePromiseVar) {
  ...
  $.ajax(settings).done(function (response, statusText, xhr) {
    ...
    if (
      ...
      logInformation(
        "Controller/Gateway group move has been initiated, please check audit trail in Central for details"
      );
    } else if (
...
function createSite() {
  ...
  if (
    ...
    showNotification(
      "ca-pin-add",
      "Please fill in either an address or coordinates to add a new Site",
      "bottom",
      "center",
      "warning"
    );
...
function unassignDeviceFromSite(device) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      ...
      data: JSON.stringify({
        ...
        site_id: parseInt(getIDforSite(device["site"])),
      }),
    }),
  };
  ...
  return $.ajax(settings).done(function (response, textStatus, jqXHR) {
    ...
    if (response.hasOwnProperty("error_code")) {
      ...
      logInformation(
        device["serial"] +
          " removed from existing site (" +
          device["site"] +
          ")"
      );
    } else {
      logError(
        "Unable to remove " + device["serial"] + " from it's current site"
      );
    }
  });
}
    ...
    if (this["SERIAL"] && this["SITE"]) {
      ...
      if (!currentSite) {
        logError(
          "Device with Serial Number: " +
            currentSerial +
            " has no site name in the CSV file"
        );
        moveCounter = moveCounter + 1;
        checkForSiteMoveCompletion();
      } else {
        ...
        if (!foundDevice) {
          logError(
            "Device with Serial Number: " +
              currentSerial +
              " was not found in the device monitoring"
          );
          moveCounter = moveCounter + 1;
          checkForSiteMoveCompletion();
        } else {
          if (!foundDevice["site"]) {
            ...
            if (siteId != -1) {
              ...
              logError(
                "Device with Serial Number: " +
                  currentSerial +
                  " could not be assigned to an unknown site"
              );
              moveCounter = moveCounter + 1;
              checkForSiteMoveCompletion();
            }
...
function renameDevices() {
  ...
  inventoryPromise = new $.Deferred();
  $.when(updateInventory()).then(function () {
    renameNotification = showProgressNotification(
      "ca-card-update",
      "Renaming devices...",
      "bottom",
      "center",
      ...
      if (this["SERIAL"] && this["DEVICE NAME"]) {
        ...
        if (!newHostname) {
          logError(
            "Device with Serial Number: " +
              currentSerial +
              " has no device name in the CSV file"
          );
          ...
          if (!device) {
              ...
              if (response.hasOwnProperty("status")) {
                if (response.status === "503") {
                  ...
                  logError(
                    "Central Server Error (503): " +
                      response.reason +
                      " (/configuration/v1/devices/<SERIAL>/template_variables)"
                  );
                }
              } else if (
                ...
                apiErrorCount++;
                logError(
                  "Central Server Error (500): Internal Server Error (/configuration/v1/devices/<SERIAL>/template_variables)"
                );
              }
              if (response !== "Success") {
                logError(
                  "The switch " + currentSerial + " was not able to be renamed"
                );
                apiErrorCount++;
              }
              renameCounter = renameCounter + 1;
            ...
            logError(
              "The gateway " +
                currentSerial +
                " was not able to be renamed,  as gateway renaming isn't supported yet"
            );
            renameCounter = renameCounter + 1;
            checkForRenameCompletion();
          }
  ...
  $.when(updateInventory()).then(function () {
    ...
    $.each(csvData, function () {
      ...
      $.when(getAnyTopologyNeighbors(currentSerial)).then(function () {
        ...
        if (
          !currentdevice["SITE"] &&
          (newHostname.includes("{{site}}") ||
            newHostname.includes("{{initials}}"))
        ) {
          logError(
            "Device with Serial Number: " +
              currentSerial +
          ...
          !neighborSwitches[currentSerial] &&
          (newHostname.includes("{{switch}}") ||
            newHostname.includes("{{port}}"))
        ) {
          logError(
            "Device with Serial Number: " +
              currentSerial +
              " has neighbor switch information"
          );
          renameCounter = renameCounter + 1;
          checkForRenameCompletion();
        } else {
          ...
          if (!device) {
            ...
            if (!connectedSwitch["neighborName"])
              connectedSwitch["neighborName"] = "UnknownSwitch";
            if (!connectedSwitch["remotePort"])
              connectedSwitch["remotePort"] = "UnknownPort";
            ...
            newHostname = newHostname.replace(
              "{{switch}}",
              connectedSwitch["neighborName"]
            );
            newHostname = newHostname.replace(
              "{{port}}",
              connectedSwitch["remotePort"]
            );
              ...
              if (
                ...
                if (response.hostname === newHostname) {
                  ...
                  logInformation(
                    "Device " +
                      currentSerial +
                      " hostname doesn't need to be updated"
                  );
                  renameCounter = renameCounter + 1;
                  checkForRenameCompletion();
                } else {
...
function getAnyTopologyNeighbors(serial) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/topology_external_api/apNeighbors/" +
        serial,
      access_token: localStorage.getItem("access_token"),
    }),
  };
  ...
  return $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (
      ...
      logError(
        "Central Server Error (503): " +
          commandResults.reason +
          " (/topology_external_api/apNeighbors/<SERIAL>)"
      );
      apiErrorCount++;
      return;
    } else if (commandResults.hasOwnProperty("error_code")) {
...
function getTopologyNeighbors(serial) {
  ...
  return $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    $.each(neighbors, function () {
      // Neighbour is a switch, and AP connects on Eth0, and its one of our managed switches
      if (
        this.neighborRole === "Switch" &&
        this.localPort === "eth0" &&
        findDeviceInMonitoring(this.neighborSerial)
      ) {
        neighborSwitches[serial] = this;
        //console.log(this)
        return false;
...
function getSwitchPorts(currentSerial) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/configuration/v1/aos_switch/ports/devices/" +
        currentSerial,
      access_token: localStorage.getItem("access_token"),
    }),
  };
  ...
  return $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (
      ...
      logError(
        "Central Server Error (503): " +
          commandResults.reason +
          " (/configuration/v1/aos_switch/ports/devices/<SERIAL>)"
      );
      apiErrorCount++;
      return;
    } else if (commandResults.hasOwnProperty("error_code")) {
...
function updateSwitchPorts(currentSerial) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/configuration/v1/aos_switch/ports/devices/" +
        neighborSwitches[currentSerial].neighborSerial,
      access_token: localStorage.getItem("access_token"),
      data: JSON.stringify({
        ports:
          modifiedUISwitches[neighborSwitches[currentSerial].neighborSerial]
            .ports,
      }),
    }),
  };
  ...
  $.ajax(settings).done(function (response, textStatus, jqXHR) {
    ...
    logInformation(
      "Updating switch port name for AP (" + currentSerial + "): " + response
    );
    if (response !== "Success") {
      logError(
        "The switch port for AP (" +
          currentSerial +
          ") was not able to be renamed"
      );
      apiErrorCount++;
    }
...
function updatePortDescription(magic) {
  ...
  showNotification(
    "ca-card-update",
    "Renaming switch ports for connected APs...",
    "bottom",
    "center",
    "info"
  );
    ...
    if (deviceType === "IAP") {
      $.when(getTopologyNeighbors(currentSerial)).then(function () {
        if (!neighborSwitches[currentSerial]) {
          ...
          var neighborSwitch = findDeviceInMonitoring(
            neighborSwitches[currentSerial].neighborSerial
          );
          var switchGroup = {};
          $.each(groups, function () {
          ...
          if (switchGroup["template_details"]["Wired"]) {
            ...
            var portName =
              "int" + neighborSwitches[currentSerial].remotePort + "_name";
            var variables = {};
            if (magic) {
            ...
            if (
              ...
              $.each(
                ...
                function () {
                  if (
                    this["port_id"] ===
                    neighborSwitches[currentSerial].remotePort.toString()
                  ) {
                    this["name"] = hostname;
              ...
              $.when(
                getSwitchPorts(neighborSwitches[currentSerial].neighborSerial)
              ).then(function () {
            ...
            logError(
              "The switch port for AP (" +
                currentSerial +
                ") was not able to be renamed as it is a CX Switch in a UI Group"
            );
            apiErrorCount++;
            updatePortsCounter = updatePortsCounter + 1;
            checkForUpdatePortCompletion();
...
function updateDeviceVariables() {
  ...
  $.each(csvData, function () {
    ...
    $.ajax(settings).done(function (response, textStatus, jqXHR) {
      ...
      if (response !== "Success") {
        logError(
          "The variables for " + currentSerial + " were not able to be updated"
        );
        apiErrorCount++;
      }
...
function setAPZone() {
  ...
  inventoryPromise = new $.Deferred();
  $.when(updateInventory()).then(function () {
    zoneNotification = showProgressNotification(
      "ca-setup-tools",
      "Setting AP Zones...",
      "bottom",
      "center",
      ...
      if (this["SERIAL"]) {
        ...
        if (!newZonename || newZonename === "-") {
          ...
          logInformation(
            "Device with Serial Number: " +
              currentSerial +
              " has no AP Zone in the CSV file"
          );
            ...
            $.ajax(settings).done(function (commandResults, statusText, xhr) {
              ...
              if (response.hasOwnProperty("error_code")) {
                ...
                logInformation(
                  "Device " +
                    currentSerial +
                    " AP Zone doesn't need to be updated"
                );
                zoneCounter = zoneCounter + 1;
                checkForZoneCompletion();
              } else {
...
function setRFProfile() {
  ...
  inventoryPromise = new $.Deferred();
  $.when(updateInventory()).then(function () {
    profileNotification = showProgressNotification(
      "ca-antenna",
      "Setting RF Profiles...",
      "bottom",
      "center",
      ...
      if (this["SERIAL"]) {
        ...
        if (!newProfileName || newProfileName === "-") {
          ...
          logInformation(
            "Device with Serial Number: " +
              currentSerial +
              " has no RF Profile in the CSV file"
          );
          rfCounter++;
          ...
          if (!device) {
            ...
            var settings = {
              ...
              data: JSON.stringify({
                url:
                  localStorage.getItem("base_url") +
                  "/configuration/v1/ap_settings_cli/" +
                  currentSerial,
                access_token: localStorage.getItem("access_token"),
              }),
            };
            ...
            $.ajax(settings).done(function (commandResults, statusText, xhr) {
              ...
              if (
                ...
                logError(
                  "Central Server Error (503): " +
                    commandResults.reason +
                    " (/configuration/v1/ap_settings_cli/<SERIAL>)"
                );
                apiErrorCount++;
                return;
              } else if (commandResults.hasOwnProperty("error_code")) {
              ...
              if (response.hasOwnProperty("error_code")) {
                ...
                $.ajax(settings).done(function (response, statusText, xhr) {
                  ...
                  if (response !== currentSerial) {
                    logError(
                      currentSerial +
                        ' was not assigned the RF Profile "' +
                        newProfileName +
                        '". Reason: ' +
                        response.reason
                    );
...
function setRadioMode(specificMode) {
  ...
  inventoryPromise = new $.Deferred();
  $.when(updateInventory()).then(function () {
    radioNotification = showProgressNotification(
      "ca-router",
      "Setting Radio Mode...",
      "bottom",
      "center",
    ...
    $.each(csvData, function () {
      ...
      if (this["SERIAL"]) {
        ...
        if (
          (!newRadio0Mode || newRadio0Mode === "-") &&
          (!newRadio1Mode || newRadio1Mode === "-") &&
          (!newRadio2Mode || newRadio2Mode === "-")
        ) {
          // "-" zone comes from the downloaded CSV from Central - equals to no configured zone.
          logInformation(
            "Device with Serial Number: " +
              currentSerial +
              " has no Radio Mode in the CSV file"
          );
          radioModeCounter++;
          ...
          if (!device) {
            ...
            $.ajax(settings).done(function (commandResults, statusText, xhr) {
              ...
              if (response.hasOwnProperty("error_code")) {
                ...
                var newSettingsKey = [
                  apCLIResponse[0].replace("per-ap-settings ", ""),
                ];
                ...
                if (
                  ...
                  if (apCLIResponse.indexOf("  radio-0-disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  radio-0-disable"),
                      1
                    );
                  if (apCLIResponse.indexOf("  dot11a-radio-disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  dot11a-radio-disable"),
                      1
                    );
                  ...
                  if (
                    newRadioMode === "access" ||
                    newRadioMode === "monitor" ||
                    newRadioMode === "spectrum"
                  ) {
                    apCLIResponse.push("  wifi0-mode " + newRadioMode);
                  } else if (newRadioMode === "off") {
                    if (
                      device.model.includes("635") ||
                      device.model.includes("655")
                    )
                      apCLIResponse.push("  radio-0-disable");
                ...
                if (
                  newRadio1Mode &&
                  newRadio1Mode !== "-" &&
                  (specificMode === "all" || specificMode === "dual")
                ) {
                  // remove the old settings
                  if (apCLIResponse.indexOf("  radio-1-disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  radio-1-disable"),
                      1
                    );
                  if (apCLIResponse.indexOf("  dot11g-radio-disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  dot11g-radio-disable"),
                      1
                    );
                  for (i = 0; i < apCLIResponse.length; i++) {
                  ...
                  if (
                    ...
                    if (
                      ...
                      apCLIResponse.push("  radio-1-disable");
                ...
                if (
                  newRadio2Mode &&
                  newRadio2Mode !== "-" &&
                  (specificMode === "all" || specificMode === "split")
                ) {
                  // remove the old settings
                  if (apCLIResponse.indexOf("  radio-2-disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  radio-2-disable"),
                      1
                    );
                  for (i = 0; i < apCLIResponse.length; i++) {
                ...
                if (
                  ...
                  // remove the old settings
                  if (apCLIResponse.indexOf("  dual-5GHz-mode disable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  dual-5GHz-mode disable"),
                      1
                    );
                  if (apCLIResponse.indexOf("  dual-5GHz-mode enable") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  dual-5GHz-mode enable"),
                      1
                    );

                  // add in the new config but only on supported AP models
                  if (
                    device.model.includes("344") ||
                    device.model.includes("345")
                  ) {
                    ...
                    if (
                      ...
                      newRadioMode === "no" ||
                      newRadioMode === "n" ||
                      newRadioMode === "disabled"
                    )
                      newRadioMode = "disable";
                    if (
                      newRadioMode === "enable" ||
                      newRadioMode === "disable"
                    ) {
                ...
                if (
                  ...
                  // remove the old settings
                  if (apCLIResponse.indexOf("  split-5ghz-mode enabled") != -1)
                    apCLIResponse.splice(
                      apCLIResponse.indexOf("  split-5ghz-mode enabled"),
                      1
                    );
                ...
                $.ajax(settings).done(function (response, statusText, xhr) {
                  ...
                  if (response !== currentSerial) {
                    logError(
                      currentSerial +
                        " was not assigned the new Radio Modes. Reason: " +
                        response.reason
                    );
                    //console.log(response.reason);
                    apiErrorCount++;
                    ...
                    logInformation(
                      currentSerial + " was assigned the new Radio Modes "
                    );
                  }
                  radioModeCounter++;
                  checkForRadioModeCompletion();
...
function setAntennaGain() {
  ...
  inventoryPromise = new $.Deferred();
  $.when(updateInventory()).then(function () {
    antennaNotification = showProgressNotification(
      "ca-antenna",
      "Setting Antenna Gains...",
      "bottom",
      "center",
      "info"
    );
    if (csvData.length == 0)
      showNotification(
        "ca-router",
        "No external antenna APs available",
        "bottom",
        "center",
        "warning"
      );
    $.each(csvData, function () {
      ...
      if (this["SERIAL"]) {
        ...
        if (
          (!newRadio0Gain || newRadio0Gain === "-") &&
          (!newRadio1Gain || newRadio1Gain === "-")
        ) {
          // "-" zone comes from the downloaded CSV from Central - equals to no configured zone.
          logInformation(
            "Device with Serial Number: " +
              currentSerial +
              " has no antenna gain in the CSV file"
          );
          antennaCounter++;
              ...
              if (response.hasOwnProperty("error_code")) {
                ...
                $.ajax(settings).done(function (response, statusText, xhr) {
                  ...
                  if (response !== currentSerial) {
                    logError(
                      currentSerial +
                        " was not assigned the new Antenna gain values. Reason: " +
                        response.reason
                    );
                    //console.log(response.reason);
                    apiErrorCount++;
                    ...
                    logInformation(
                      currentSerial +
                        " was assigned the new Antenna gain values"
                    );
                  }
                  antennaCounter++;
                  checkForAntennaCompletion();
        ...
        if (!device) {
          ...
          $.ajax(settings).done(function (commandResults, statusText, xhr) {
            ...
            if (response.hasOwnProperty("error_code")) {
              ...
              // no need to do anything as the name already matches
              logInformation(
                "Device " + currentSerial + " 2.4GHz radio is already disabled"
              );
              radioCounter = radioCounter + 1;
              checkForRadioCompletion();
            } else {
              ...
              $.ajax(settings).done(function (response, textStatus, jqXHR) {
                ...
                if (response !== currentSerial) {
                  ...
                  logInformation(
                    '2.4GHz radio on AP "' + currentSerial + '" was disabled'
                  );
                }
                radioCounter = radioCounter + 1;
                checkForRadioCompletion();
        ...
        if (!device) {
          ...
          $.ajax(settings).done(function (commandResults, statusText, xhr) {
            ...
            if (response.hasOwnProperty("error_code")) {
              ...
              // no need to do anything as the name already matches
              logInformation(
                "Device " + currentSerial + " 2.4GHz radio is already enabled"
              );
              radioCounter = radioCounter + 1;
              checkForRadioCompletion();
            } else {
              ...
              $.ajax(settings).done(function (response, textStatus, jqXHR) {
                ...
                if (response !== currentSerial) {
                  ...
                  logInformation(
                    '2.4GHz radio on AP "' + currentSerial + '" was enabled'
                  );
                }
                radioCounter = radioCounter + 1;
                checkForRadioCompletion();
...
function renameAndRadioMode() {
  ...
  $.when(updateInventory()).then(function () {
    showNotification(
      "ca-contactless-card",
      "Setting Hostname and Radio Mode...",
      "bottom",
      "center",
      "info"
    );
    ...
    $.each(csvData, function () {
      ...
      if (this["SERIAL"]) {
        ...
        if (
          ...
          (!newRadio1Mode || newRadio1Mode === "-") &&
          (!newRadio2Mode || newRadio2Mode === "-") &&
          (!newAPHostname || newAPHostname === "-")
        ) {
          // "-" zone comes from the downloaded CSV from Central - equals to no configured zone.
          logInformation(
            "Device with Serial Number: " +
              currentSerial +
              " has no Hostname AND Radio Mode in the CSV file"
          );
          radioModeCounter++;
            ...
            $.ajax(settings).done(function (commandResults, statusText, xhr) {
              ...
              if (response.hasOwnProperty("error_code")) {
                ...
                if (newAPHostname && newAPHostname !== "-") {
                  ...
                  var hostnameIndex = -1;
                  for (i = 0; i < apCLIResponse.length; i++) {
                    if (apCLIResponse[i].includes("hostname"))
                      hostnameIndex = i;
                  }
                  if (hostnameIndex != -1)
                    apCLIResponse.splice(hostnameIndex, 1);
                ...
                $.ajax(settings).done(function (response, statusText, xhr) {
                  ...
                  if (response !== currentSerial) {
                    logError(
                      currentSerial +
                        " was not assigned the new Hostname and Radio Modes. Reason: " +
                        response.reason
                    );
                    //console.log(response.reason);
                    apiErrorCount++;
                    ...
                    logInformation(
                      currentSerial +
                        " was assigned the new Hostname and Radio Modes "
                    );
                  }
                  radioModeCounter++;
                  checkForNameAndRadioModeCompletion();
...
function selectCustomer() {
  ...
  document.getElementById("manualCustomerParagraph").innerText =
  ...
  showNotification(
    "ca-folder-replace",
    manualCustomer +
      " will be used for devices with no customer assigned. Please re-run the task.",
    "top",
    "center",
    "warning"
  );
...
function isMSP(offset) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/msp_api/v1/devices?limit=1&offset=0&device_allocation_status=0&device_type=iap",
      access_token: localStorage.getItem("access_token"),
    }),
  };
...
function getMSPResources() {
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.branding.logo_image_url) {
      document.getElementById("msp_logo").src =
        response.branding.logo_image_url;
...
function getMSPAPData(offset) {
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/device_inventory/v1/msp/devices?limit=" +
        apiMSPLimit +
        "&offset=" +
        offset +
        "&device_allocation_status=0&device_type=iap",
      access_token: localStorage.getItem("access_token"),
    }),
    complete: function () {
  ...
  $.each(mspAPs, function () {
    ...
    if (monitoringInfo) {
      ...
      table.row.add([
        ...
        monitoringInfo.ip_address ? monitoringInfo.ip_address : "",
        monitoringInfo.name ? monitoringInfo.name : "",
        monitoringInfo.group_name ? monitoringInfo.group_name : "",
        monitoringInfo.site ? monitoringInfo.site : "",
        this.tier_type ? titleCase(this.tier_type) : "",
      ]);
    } else {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        ...
        "&offset=" +
        offset +
        "&device_allocation_status=0&device_type=switch",
      access_token: localStorage.getItem("access_token"),
    }),
    complete: function () {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        ...
        "&offset=" +
        offset +
        "&device_allocation_status=0&device_type=all_controller",
      access_token: localStorage.getItem("access_token"),
    }),
    complete: function () {
...
function getMSPCustomerData(offset) {
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (document.getElementById("customer_count"))
        document.getElementById("customer_count").innerHTML = "-";
    } else if (response.hasOwnProperty("message")) {
      showNotification(
        "ca-globe",
        response.message,
...
function unassignDeviceFromCustomer(device) {
  ...
  var settings = {
    ...
    data: JSON.stringify({
      url:
        localStorage.getItem("base_url") +
        "/platform/device_inventory/v1/msp/" +
        mspID +
        "/devices",
      access_token: localStorage.getItem("access_token"),
      data: JSON.stringify({
        devices: [{ serial: device["serial"], mac: device["macaddr"] }],
      }),
    }),
  };
    ...
    if (response.status != 202) {
      if (response.status === "503") {
        ...
        logError(
          "Central Server Error (503): " +
            response.reason +
            " (/platform/device_inventory/v1/msp/<MSP_ID>/devices)"
        );
      } else if (response.error_code) {
        logError(
          titleCase(noUnderscore(response.error_code)) +
            " for device: " +
            deviceSerial
        );
      } else {
  ...
  return $.ajax(settings).done(function (response, textStatus, jqXHR) {
    if (response.status != 202) {
      if (response.status === "503") {
        ...
        logError(
          titleCase(noUnderscore(response.error_code)) + " for supplied devices"
        );
      } else {
        logError(response.reason);
      }
...
function assignDevicesToCustomer() {
  ...
  $.each(csvData, function () {
    ...
    if (foundDevice && foundDevice.customer_name === selectedCustomer) {
      ...
      foundDevice &&
      foundDevice.customer_name !== selectedCustomer &&
      getIDforCustomer(foundDevice.customer_name) != -1
    ) {
...
function assignAllDevicesToCustomer() {
  ...
  showNotification(
    "ca-exchange",
    "Assigning devices to a single customer...",
    "bottom",
    "center",
    "info"
  );
  ...
  if (unassignDevicesArray.length != 0) {
    $.when(unassignDevicesFromCustomers(unassignDevicesArray)).then(
      function () {
        // now assign to new Customer ID
        console.log("Assigning all devices to " + selectedCustomer);
        assignDevicesToSingleCustomer(
          devicesArray,
          getIDforCustomer(selectedCustomer)
        );
      }
    );
  } else {
...
function createCustomer() {
  ...
  var customerDescription = document.getElementById(
    "customerDescription"
  ).value;
...
function buildMSPCSVData() {
  ...
  $.each(mspAPs, function () {
    ...
    if (monitoringInfo) {
      var groupToUse = monitoringInfo["group_name"]
        ? monitoringInfo["group_name"]
        : "";
      ...
      csvDataBuild.push({
        [customerKey]: device["customer_name"],
        [serialKey]: device["serial"],
        [macKey]: device["macaddr"],
        [typeKey]: device["device_type"],
        [skuKey]: device["aruba_part_no"],
        [modelKey]: device["model"],
        [statusKey]: monitoringInfo["status"] ? monitoringInfo["status"] : "",
        [uptimeKey]: duration.humanize(),
        [ipKey]: monitoringInfo["ip_address"]
          ? monitoringInfo["ip_address"]
          : "",
        [nameKey]: monitoringInfo["name"] ? monitoringInfo["name"] : "",
        [groupKey]: groupToUse,
        [siteKey]: siteToUse,
        [licenseKey]: device["tier_type"] ? titleCase(device["tier_type"]) : "",
      });
    } else {
      var groupToUse = "";
      var siteToUse = "";
...
function getSwitchType(serial) {
  ...
  if (
    currentSwitch["model"].includes("60") ||
...
function showLayerOne() {
  showNotification(
    "ca-cable",
    "Finding LLDP Neighbours of every AP at the Site...",
    "bottom",
    "center",
    "info"
  );
  $("#layerone-table").DataTable().rows().remove();
  $("#layerone-table").DataTable().clear();
    ...
    if (this["mesh_role"] !== "Point") {
      $.when(getTopologyNeighbors(currentSerial)).then(function () {
        if (!neighborSwitches[currentSerial]) {
          ...
          $.when(
            getSwitchPortDetails(neighborSwitches[currentSerial].neighborSerial)
          ).then(function () {
            $.each(
              switchPortDetails[neighborSwitches[currentSerial].neighborSerial],
              function () {
                if (
                  this["port_number"] ===
                  neighborSwitches[currentSerial].remotePort
                ) {
                  // Buttons for each row
                  var btnString =
                    '<a class="btn btn-link btn-warning" data-toggle="tooltip" data-placement="top" title="PoE Bounce" onclick="poeBounce(\'' +
                    neighborSwitches[currentSerial].neighborSerial +
                    "', '" +
                    this["port_number"] +
                    '\')"><i class="fa-solid fa-plug-circle-bolt"></i></a>';
                  btnString +=
                    '<a class="btn btn-link btn-warning" data-toggle="tooltip" data-placement="top" title="Interface Bounce" onclick="interfaceBounce(\'' +
                    neighborSwitches[currentSerial].neighborSerial +
                    "', '" +
                    this["port_number"] +
                    '\')"><i class="fa-solid fa-ethernet"></i></a>';
                  if (
                    getSwitchType(
                      neighborSwitches[currentSerial].neighborSerial
                    ) === "AOS-S"
                    ...
                    btnString +=
                      '<a class="btn btn-link btn-warning" data-toggle="tooltip" data-placement="top" title="Cable Test" onclick="cableTest(\'' +
                      neighborSwitches[currentSerial].neighborSerial +
                      "', '" +
                      this["port_number"] +
                      '\')"><i class="fa-solid fa-microscope"></i></a>';
                  }
                  ...
                  table.row.add([
                    ...
                    status,
                    neighborSwitches[currentSerial].neighborHostName,
                    this["port_number"],
                    poe,
                    this["speed"],
                    this["duplex_mode"],
...
function poeBounce(switchSerial, switchPort) {
  ...
  if (getSwitchType(switchSerial) === "AOS-S") {
    data = JSON.stringify({
      device_type: "switch",
      commands: [
        { command_id: 1050, arguments: [{ name: "Port", value: switchPort }] },
      ],
    });
  } else {
    data = JSON.stringify({
      device_type: "switch",
      commands: [
        { command_id: 6003, arguments: [{ name: "Port", value: switchPort }] },
      ],
    });
  }
...
function interfaceBounce(switchSerial, switchPort) {
  ...
  if (getSwitchType(switchSerial) === "AOS-S") {
    data = JSON.stringify({
      device_type: "switch",
      commands: [
        { command_id: 1051, arguments: [{ name: "Port", value: switchPort }] },
      ],
    });
  } else {
    data = JSON.stringify({
      device_type: "switch",
      commands: [
        { command_id: 6004, arguments: [{ name: "Port", value: switchPort }] },
      ],
    });
  }
...
function confirmCableTest(switchSerial, switchPort) {
  ...
  if (getSwitchType(switchSerial) === "AOS-S") {
    data = JSON.stringify({
      device_type: "switch",
      commands: [
        { command_id: 1205, arguments: [{ name: "Port", value: switchPort }] },
      ],
    });
  } else {
  ...
  $.ajax(settings).done(function (commandResults, statusText, xhr) {
    ...
    if (response.hasOwnProperty("error")) {
      ...
      if (response.status === "RUNNING" || response.status === "QUEUED") {
        showNotification(
          "ca-window-code",
          response.message.replace(" Please try after sometime", "."),
          "bottom",
          "center",
          "info"
        );
        ...
        if (response.output.includes("cable_status")) {
          ...
          Swal.fire({
            title:
              findDeviceInMonitoring(response.serial).name +
              " Cable Test - Port: " +
              switchPort,
...
function rebootDevices() {
  ...
  rebootNotification = showProgressNotification(
    "ca-reload",
    "Rebooting devices...",
    "bottom",
    "center",
  ...
  $.each(csvData, function () {
    ...
    if (!this["SERIAL"]) {
      ...
      $.ajax(settings).done(function (response) {
        ...
        if (response.hasOwnProperty("status")) {
          if (response.status === "503") {
            logError(
              "Central Server Error (503): " +
                response.reason +
                " (/device_management/v1/device/<SERIAL>/action/reboot)"
            );
            return;
          }
        }
        ...
        var rebootProgress =
          ((rebootedDevices + rebootErrors + skippedDevices) / csvData.length) *
          100;
          ...
          if (rebootErrors > 0) {
            ...
            Swal.fire({
              ...
              text:
                "Some or all devices (" +
                rebootErrors +
                ") failed to be rebooted",
              icon: "error",
            });
            if (skippedDevices > 0)
              console.log(
                "CSV file contained " + skippedDevices + " empty rows"
              );
          } else {
            Swal.fire({
